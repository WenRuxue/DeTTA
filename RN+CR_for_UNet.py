...
def forward_and_adapt(x, model, optimizer):
    ...
    outputs = model(x)
    # adapt
    maxpool = nn.MaxPool2d(k)
    outputs = maxpool(outputs)

    V = outputs.flatten(start_dim=1)
    loss_RN = -((V.t()).mm(V)).trace()

    maxpool_d = nn.MaxPool2d(2*d+1)

    C = maxpool_d(outputs)+maxpool_d(-outputs)

    loss_CR = torch.norm(C, dim=[2,3]).sum()

    loss = alpha*loss_RN + loss_CR
    loss.backward()
    optimizer.step()
    optimizer.zero_grad()
    return outputs
